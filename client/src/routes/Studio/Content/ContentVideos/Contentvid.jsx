...
const loginData = useSelector(s=>s.loginData)
  ...
  const getVid = async () =>  {
    try {
        const res =   await fetch(`https://youtube-clone-v8xu.onrender.com/api/videos/getuploaded_vid/${loginData?.user?._id}`)
...
console.log(vid)
  ...
  const res = await fetch(`https://youtube-clone-v8xu.onrender.com/api/videos/delete_selected_vid/${vid}` , {
    method:"DELETE"
  } )
  const  data  = await res.json()
  alert(data?.message)
